...
# Adjust this to the actual file path
file_path = "new_memory.csv"
df = pd.read_csv(file_path)

# Group by algorithm and take the mean of communication cost and memory
grouped_by_algo = (
...
for i in range(1, 11):
    plt.plot(
        grouped_by_algo["Algorithm"],
        grouped_by_algo[f"Pretrain Network Large{i}"],
        label=f"Pretrain Network Large{i}",
        marker="o",
    )
...
for i in range(1, 11):
    plt.plot(
        grouped_by_algo["Algorithm"],
        grouped_by_algo[f"Train Network Large{i}"],
        label=f"Train Network Large{i}",
        marker="x",
    )
...
for i in range(1, 11):
    plt.plot(
        grouped_by_algo["Algorithm"],
        grouped_by_algo[f"Pretrain Max Trainer Memory{i}"],
        label=f"Pretrain Max Trainer Memory {i}",
        marker="o",
    )
...
for i in range(1, 11):
    plt.plot(
        grouped_by_algo["Algorithm"],
        grouped_by_algo[f"Train Max Trainer Memory{i}"],
        label=f"Train Max Trainer Memory {i}",
        marker="x",
    )
