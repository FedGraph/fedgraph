...
def process_log(log_content):
    ...
    for line in log_content.splitlines():
        ...
        if pretrain_network_match and pretrain_mode:
            current_experiment[f"Pretrain Network {pretrain_network_match.group(1)}"
        ...
        train_max_trainer_memory_match = re.search(r"Log Max memory for Large(\d+): (\d+\.\d+)", line
        )
