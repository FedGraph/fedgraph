...
def plot_combined_charts(df, iid_beta_value, hop_value):
    ...
    pretrain_values = batch_data.groupby(
        "Batch Size")["Pretrain Time"].mean() if hop_value == 1 else None
    train_values = batch_data.groupby("Batch Size")["Train Time"].mean()
    pre_columns = [f"Pretrain Network Large{i}" for i in range(1, 11)]
    pre_values = batch_data[pre_columns].sum(
        axis=1).groupby(batch_data["Batch Size"]).mean()
    tre_columns = [f"Train Network Large{i}" for i in range(1, 11)]
    tre_values = batch_data[tre_columns].sum(
        axis=1).groupby(batch_data["Batch Size"]).mean()
    accuracy_values = batch_data.groupby(
        "Batch Size")["Average Test Accuracy"].mean()
    ...
    if hop_value == 1:
        ax1.bar(batch_size_range - width/2, pretrain_values.values,
                width=width, label="Pretrain Time", color="skyblue")
    ax1.bar(batch_size_range + (width/2 if hop_value == 1 else 0),
            train_values.values, width=width, label="Train Time", color="orange")
    ax1.set_title(f"Pretrain vs Train Time (IID Beta {iid_beta_value}, Hop {hop_value})")
    ...
    if hop_value == 1:
        ax2.bar(batch_size_range - width/2, pre_values.values,
                width=width, label="Pretrain Communication Cost", color="skyblue")
    ax2.bar(batch_size_range + (width/2 if hop_value == 1 else 0),
            tre_values.values, width=width, label="Train Communication Cost", color="orange")
    ax2.set_title(f"Total Communication Cost (IID Beta {iid_beta_value}, Hop {hop_value})")
    ...
    # Plot Accuracy
    ax3.bar(batch_size_range, accuracy_values.values,
