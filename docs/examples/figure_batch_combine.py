...
def filter_data(df, iid_beta_value, hop_value):
    return df[(df["IID Beta"] == iid_beta_value) & (df["Number of Hops"] == hop_value) & (df["Batch Size"].isin(batch_sizes))]
...
def plot_accuracy_comparison(df, hop_value):
    ...
    for i, iid_beta_value in enumerate(iid_beta_values):
        ...
        accuracy_values = filtered_df.groupby(
        ...
        plt.bar(batch_size_range + i * width, accuracy_values.values,
                width=width, label=f"IID Beta {iid_beta_value}")
