...
class TestFedGraphNCWorkflow:
    ...
    def setup_method(self):
        ...
        if sum(len(split) for split in self.split_node_indexes) < self.num_nodes:
            ...
            self.split_node_indexes[-1] = torch.cat(
                [self.split_node_indexes[-1], remainder]
            )
    ...
    def test_nc_data_processing_pipeline(
        ...
        mock_partition.return_value = [
            split.tolist() for split in self.split_node_indexes
        ]
...
class TestFedGraphGCWorkflow:
    ...
    def test_gc_complete_workflow(
        self, mock_run_fed, mock_setup_trainers, mock_setup_server
    ):
    ...
    def test_gc_trainer_creation(self):
        ...
        # Mock dataloader
        mock_dataloader = {"train": Mock(), "val": Mock(), "test": Mock()}
...
class TestFedGraphLPWorkflow:
    ...
    def test_lp_basic_setup(
        self, mock_server_lp, mock_get_time, mock_get_mapping, mock_check_files
    ):
...
class TestFedGraphEndToEnd:
    ...
    def test_configuration_validation(self):
        ...
        with pytest.raises(
            ValueError,
            match="Low-rank compression currently only supported for NC tasks",
        ):
            run_fedgraph(args)
        ...
        with pytest.raises(
            ValueError, match="Low-rank compression currently only supported for NC"
        ):
            run_fedgraph(args)
    ...
    def test_data_trainer_server_interaction(self):
        ...
        with patch("fedgraph.trainer_class.GCN") as mock_gcn:
            ...
            with patch("fedgraph.server_class.AggreGCN") as mock_server_gcn:
                ...
                # Test parameter flow
                trainer.update_params = Mock()
                mock_server_model.state_dict.return_value = {
                    "weight": torch.randn(32, 20)
                }
