"""Pytest configuration shared by the FedGraph test suites.

Provides:

* Common fixtures for unit and integration tests (``temp_dir``,
  ``sample_graph_data``, ``mock_args``, etc.).
...
def sample_dataset_splits():
    ...
    for i in range(num_clients):
        client_data = {
            ...
            "node_list": list(
                range(i * num_nodes_per_client, (i + 1) * num_nodes_per_client)
            ),
        }
