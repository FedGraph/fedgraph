...
class TestResolveNCClassNum:
    ...
    def test_rejects_huggingface_data_without_labels(self):
        ...
        with pytest.raises(
            ValueError, match="all train and test label tensors are empty"
        ):
            _resolve_nc_class_num(True, trainer_information)
...
class TestRunNC:
    ...
    def setup_method(self):
        ...
        self.mock_data = (
            ...
            7,  # class_num
            [
                torch.arange(i * 10, (i + 1) * 10) for i in range(3)
            ],  # split_node_indexes
            {
                i: torch.arange(i * 20, (i + 1) * 20) for i in range(3)
...
class TestRunGC:
    ...
    @patch("fedgraph.federated_methods.Monitor")
    def test_run_gc_basic_setup(
        self, mock_monitor, mock_setup_trainers, mock_setup_server
    ):
    ...
    def test_run_gc_gcfl(self, mock_run_gcfl):
        ...
        with patch(
            "fedgraph.federated_methods.setup_server"
...
class TestRunLP:
    ...
    def test_run_lp_basic_setup(
        self, mock_get_time, mock_get_mapping, mock_check_files
    ):
...
class TestFederatedMethodsIntegration:
    ...
    def test_task_routing(
        self, mock_run_lp, mock_run_gc, mock_run_nc, mock_data_loader
    ):
