...
class TestNCLoadData:
    ...
    def test_load_cora_dataset(
        self, mock_makedirs, mock_parse, mock_pkl_load, mock_open_file, mock_download
    ):
...
class TestDataLoaderGCSingle:
    ...
    def test_data_loader_GC_single_success(
        self, mock_get_labels, mock_dataloader, mock_split, mock_chunk, mock_tudataset
    ):
        ...
        result = data_loader_GC_single(
            datapath="/tmp", dataset="PROTEINS", num_trainer=5, batch_size=32, seed=42
        )
...
class TestDataLoaderGCMultiple:
    ...
    @patch("fedgraph.data_process.DataLoader")
    @patch("fedgraph.data_process.get_num_graph_labels")
    def test_data_loader_GC_multiple_small_group(
        self, mock_get_labels, mock_dataloader, mock_split, mock_tudataset
    ):
        ...
        result = data_loader_GC_multiple(
            datapath="/tmp", dataset_group="small", batch_size=32, seed=42
        )
...
class TestDataLoaderNC:
    ...
    def test_data_loader_NC_complete_flow(
        self, mock_get_indexes, mock_partition, mock_load_data
    ):
        ...
        mock_get_indexes.return_value = (
            {
                i: torch.arange(i * 10, (i + 1) * 10) for i in range(3)
...
class TestDataProcessIntegration:
    ...
    def test_complete_NC_workflow_mock(self):
        ...
        with patch("fedgraph.data_process.NC_load_data") as mock_load, patch(
            "fedgraph.data_process.label_dirichlet_partition"
