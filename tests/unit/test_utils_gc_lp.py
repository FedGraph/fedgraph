...
class TestUtilsGC:
    ...
    def test_setup_trainers(self, mock_trainer_gc):
        ...
        mock_model = Mock()
        mock_data = {
            "trainer_0": ({"train": Mock(), "val": Mock(), "test": Mock()}, 10, 2, 100),
            "trainer_1": ({"train": Mock(), "val": Mock(), "test": Mock()}, 10, 2, 100),
        }
...
class TestUtilsIntegration:
    ...
    def test_lp_data_workflow(self):
        ...
        with patch("fedgraph.utils_lp.get_data") as mock_get_data:
            ...
            user_mapping, item_mapping = get_global_user_item_mapping(
                "ml-1m", "/fake/path"
            )
