...
class Monitor:
    def __init__(self):
        ...
        self.pretrain_node_network_gauge = Gauge(
            "pretrain_node_network", description="Network data sent during training per pod."
        )
        self.train_node_network_gauge = Gauge(
            "train_node_network", description="Network data sent during training per pod."
        )
        self.pretrain_start_time = None
        self.pretrain_end_time = None
        self.train_start_time = None
        self.train_end_time = None
        self.current_round = 0
        self.initial_network_data = {}
        self.final_network_data = {}

    def _get_network_data(self):
        response = requests.get("http://prometheus-kube-prometheus-prometheus.prometheus-system:9090/api/v1/query?query=ray_node_network_sent")
        data = response.json()
        return {item['metric']['pod']: float(item['value'][1]) for item in data['data']['result']}

    def pretrain_time_start(self):
    ...
    def pretrain_time_end(self):
        self.pretrain_end_time = datetime.datetime.now()
        pretrain_duration = (self.pretrain_end_time - self.pretrain_start_time).total_seconds() * 1000
        ...
        for pod in self.final_network_data:
            if pod in self.initial_network_data:
                network_diff = self.final_network_data[pod] - self.initial_network_data[pod]
    ...
    def train_time_end(self):
        self.train_end_time = datetime.datetime.now()
        train_duration = (self.train_end_time - self.train_start_time).total_seconds() * 1000
        ...
        for pod in self.final_network_data:
            if pod in self.initial_network_data:
                ...
                print(pod, network_diff)
                break
        print("Train end time recorded, duration set to gauge, and network data difference calculated.")
