...
class Trainer_General_LowRank(Trainer_General):
    ...
    def get_compressed_params(self) -> Dict[str, Any]:
        ...
        params = {name: param.data.cpu().detach() for name, param in self.model.named_parameters()}
        ...
        for name, param in params.items():
            if param.dim() == 2 and min(param.shape) > 1:
                ...
                if rank > max_possible_rank:
                    print(f"Warning: rank {rank} > max possible {max_possible_rank} for {name}, using {max_possible_rank}")
    ...
    def update_compressed_params(self, compressed_data: Dict[str, Any], 
                               current_global_epoch: int) -> None:
        """
