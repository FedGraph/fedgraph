...
from .compression_utils import svd_compress, svd_decompress, auto_select_rank
...
class Server_LowRank(Server):
    ...
    def compress_params(self, params: Dict[str, torch.Tensor]) -> Dict[str, Any]:
        ...
        return {'params': compressed_params, 'compressed': True, 'info': compression_info}

    def decompress_params(self, compressed_data: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        """
    ...
    @torch.no_grad()
    def train(self, current_global_epoch: int, sampling_type: str = "random", 
        ...
        if self.use_lowrank:
            ...
            aggregated_compressed = self.aggregate_compressed_params(params, num_samples)
    ...
    def aggregate_compressed_params(self, params_list: List, num_samples: int) -> Dict[str, Any]:
        """
        ...
        for name in param_names:
            ...
            if isinstance(first_param, dict) and 'U' in first_param:
                ...
                aggregated['params'][name] = param_sum / num_samples
        
        return aggregated

    def broadcast_compressed_params(self, current_global_epoch: int, 
                                  compressed_params: Dict[str, Any]) -> None:
        """
        Broadcast compressed parameters to all trainers.
        """
        for trainer in self.trainers:
            trainer.update_compressed_params.remote(compressed_params, current_global_epoch)
    ...
    def print_compression_stats(self) -> None:
        ...
        total_compressed = sum(info['compressed_size'] for info in latest_stats.values())
        ...
        for name, info in latest_stats.items():
            if info['rank'] is not None:
                print(f"{name}: {info['original_shape']} -> rank {info['rank']} "
    ...
    def get_model_size(self) -> float:
        ...
        latest_stats = self.compression_stats[-1]
        total_compressed_params = sum(info['compressed_size'] for info in latest_stats.values())
