...
def greedy_loss_fedsage_plus(pred_feats, true_feats, pred_missing, true_missing, max_pred):
    """
    Loss function for FedSAGE+ Neighbor generation model
    
    Parameters
    ----------
    pred_feats : torch.Tensor
        The prediction features from fGen
    true_feats : torch.Tensor
        The true features of missing nodes
    pred_missing : torch.Tensor
        The prediction missing nodes from dGen
    true_missing : torch.Tensor
        The true number of missing nodes
    max_pred : int
        The maximum number of missing nodes prediction
    
    Returns
    -------
    (tensor) : torch.Tensor
        The greedy loss for FedSAGE+ neighbor generation
    ...
    for i in range(num_samples):
        for pred_j in range(min(max_pred, int(pred_missing[i]))):
            if true_missing[i]>0:
                ...
                # go through all true_missing nodes, get loss
